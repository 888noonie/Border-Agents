...
impl App {
    ...
    fn update_input_region(&mut self) {
        ...
        let rects = if self.reader_saved.is_some() {
            ...
            if self.speech.is_some() {
                ...
                rects.push(self.offset_rect_for_body(layout.bubble_outer_resize_rect()).as_i32());
            }
            if self.chat_open {
    ...
    fn on_press(&mut self, x: f64, y: f64, button: u32) {
            ...
            && render::torso_action_at(&layout, body_x, y).is_none()
        {
            ...
            PressTarget::BubbleExpand
        } else if self.speech.is_some() && layout.bubble_outer_resize_rect().contains(body_x, y) {
