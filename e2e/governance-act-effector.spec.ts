 ...
 *
 * The `allow` path for an act effector requires an action-backed memory turn (a graded
 * assistant message carrying may_use_for_action), which needs a live provider response not
 * available offline — it is proven at the soul layer in src/__tests__/soulActions.test.ts
 * ("Case C"). Here we prove the block paths hold on the real UI.
 */
